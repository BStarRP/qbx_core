...
local function loadItems(oxItems)
    for item, data in pairs(oxItems) do
        qbShared.Items[item] = {
            name = item,
            label = data.label,
            weight = data.weight or 0,
            type = oxItems[item].type or 'item',
...
loadItems(oxItems)
--RegisterNetEvent('ox_inventory:itemList', loadItems)
...
qbShared.SortPairsByKeys = function(tbl)
    return qbx.sortPairsByKeys(tbl)
end

qbShared.GetVehicleModelName = function(vehicle)
    if not vehicle then return nil end
    return qbx.getVehicleModelName(vehicle)
end
